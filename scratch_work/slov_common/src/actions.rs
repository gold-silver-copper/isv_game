    ...
    for (eid, ent_type,mut e_pos, e_action) in point_action.iter_mut() {
        match e_action {
            ActionComponent::Go(direction) => {
                ...
                attempt_move_ent_to_point(&mut e_pos,&potential_loc,&mut masterok.game_map,&eid,&ent_type)
...
pub fn attempt_move_ent_to_point(origin: &mut PointComponent, destination: &MyPoint, game_map: &mut GameMap ,eid: &Entity ,ent_type:&EntityType ) {
    ...
    if !vox_blocks_movement {
      ...
      insert_ent_at_point(origin,destination,game_map,eid,ent_type)
    }
...
pub fn insert_ent_at_point(origin: &mut PointComponent, destination: &MyPoint, game_map: &mut GameMap ,eid: &Entity ,ent_type:&EntityType ) {
